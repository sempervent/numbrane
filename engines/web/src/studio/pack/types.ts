...
export function packToPerformanceSet(pack: PflPack): Record<string, unknown> {
  const scenes = pack.items
    .filter((i) => i.kind === "animation" || i.kind === "react")
    .map((item, idx) => {
      const maps =
        item.kind === "react"
          ? [
        ...
        layers: [
          {
            ...
            opacity: 1,
            seed: item.seed,
            parameters: { ...item.parameters, ...(item.styleId ? { pfl_style: item.styleId } : {}) },
          },
        ],
        modulation: maps,
  ...
  if (!scenes.length) {
    scenes.push({
      ...
      layers: [
        {
          id: "L0",
          piece: pack.items[0]?.pieceId || "geometry/metatron",
          opacity: 1,
          seed: pack.items[0]?.seed ?? 42,
          parameters: pack.items[0]?.parameters ?? { density: 0.6 },
        },
      ],
      modulation: [],
